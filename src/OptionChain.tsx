...
export default function OptionChain({ symbol, expiries, sessionToken, exchange = 'NSE', onClose, onAddLeg, onLtpUpdateRef, lotSize = 1 }: {
  symbol: string;
  ...
  // React hook dependency cleanup since onLtpUpdateRef is a ref
  ...
  // Scroll to ATM when rows load
  useEffect(() => {
    if (rows.length === 0 || !atmRowRef.current) return;
  ...
  const buildRows = (ceList: Record<string, number>[], peList: Record<string, number>[], atmRaw: number, spotRaw: number, isRest: boolean) => {
    ...
    for (const opt of ceList) {
      ...
      map.get(s)!.ce = isRest ? parseRest(opt) : parseWs(opt);
    }
    for (const opt of peList) {
      const s = (opt[sk] ?? 0) / scale;
      if (!map.has(s)) map.set(s, { strike: s, ce: { ...EMPTY }, pe: { ...EMPTY }, isAtm: false });
      map.get(s)!.pe = isRest ? parseRest(opt) : parseWs(opt);
    }
    ...
    setRows(sorted);
    if (onLtpUpdateRef && onLtpUpdateRef.current) {
      ...
      sorted.forEach(r => ltpMap.set(r.strike, {
