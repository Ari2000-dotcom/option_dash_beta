...
function UnderlyingInput({ underlyings, value, onChange }: { underlyings: string[]; value: string; onChange: (v: string) => void }) {
  ...
  return (
    <div className="flex flex-col shrink-0" ref={ref}>
      ...
      <label className="input input-sm flex items-center gap-1.5 bg-[#1E222D] border-[#2A2E39] text-[#D1D4DC] w-40 focus-within:border-[rgba(255,152,0,0.45)] focus-within:outline-none rounded-lg cursor-text">
        <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="#52525b" strokeWidth="2.5" className="shrink-0">
        ...
        <input
          ...
          onFocus={openDrop}
          placeholder="NIFTY…"
          className="grow bg-transparent outline-none text-[13px] text-[#D1D4DC] placeholder-[#4A4E5C] min-w-0"
        />
      </label>
      {open && filtered.length > 0 && dropPos && createPortal(
        <div
          ...
          className="bg-[#0f1117] border border-[rgba(255,255,255,0.12)] rounded-lg shadow-[0_16px_48px_rgba(0,0,0,0.9)] overflow-hidden"
        >
          <div className="overflow-y-auto" style={{ maxHeight: 220 }}>
            {filtered.map(u => (
              <div key={u}
                onMouseDown={(e: React.MouseEvent) => { e.preventDefault(); onChange(u); setOpen(false); }}
                className="px-3 py-2 text-[13px] text-[#D1D4DC] cursor-pointer hover:bg-[rgba(255,152,0,0.08)] hover:text-[#FF9800] transition-colors">
                {u}
              </div>
            ))}
          </div>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', flexShrink: 0 }}>
      ...
      <button
        ...
        onClick={handleOpen}
        style={{
          minWidth: 110, height: 28, display: 'inline-flex', alignItems: 'center', justifyContent: 'space-between',
          ...
          color: displayLabel ? '#D1D4DC' : '#52525b', fontSize: 12, fontWeight: 600, gap: 4,
        }}
      >
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', flexShrink: 0 }}>
      ...
      <button
        ...
        onClick={handleOpen}
        style={{
          minWidth: 72, height: 28, display: 'inline-flex', alignItems: 'center', justifyContent: 'space-between',
          padding: '0 8px', cursor: 'pointer',
          border: '1px solid rgba(255,255,255,0.12)', background: open ? 'rgba(255,255,255,0.08)' : 'rgba(255,255,255,0.04)',
          borderRadius: 6, boxSizing: 'border-box', userSelect: 'none',
          color: '#D1D4DC', fontSize: 12, fontWeight: 600, gap: 4,
        }}
      >
        ...
        return (
          <button
            ...
            style={{
              ...
              borderTop: `1px solid ${active ? color + '55' : 'rgba(255,255,255,0.12)'}`,
              borderBottom: `1px solid ${active ? color + '55' : 'rgba(255,255,255,0.12)'}`,
              borderLeft: `1px solid ${active ? color + '55' : 'rgba(255,255,255,0.12)'}`,
              borderRight: isLast ? `1px solid ${active ? color + '55' : 'rgba(255,255,255,0.12)'}` : 'none',
              borderRadius: isFirst ? '6px 0 0 6px' : isLast ? '0 6px 6px 0' : 0,
              background: active ? color + '18' : 'rgba(255,255,255,0.03)',
              color: active ? color : '#787B86',
            }}
          >
            {label}
          </button>
...
function MultiExpiryView({ instruments, visible = true, toolbarSlot }: { instruments: Instrument[]; visible?: boolean; toolbarSlot?: Element | null }) {
  ...
  useEffect(() => {
    ...
    const chart = createChart(el, { ...CHART_OPTIONS, width: initW || 600, height: initH || 400 });
...
function StraddleMulti({ instruments, straddleMode, visible = true, toolbarSlot }: { instruments: Instrument[]; straddleMode: 'single' | 'multi'; visible?: boolean; toolbarSlot?: Element | null }) {
  ...
  // Boot chart — containerRef is position:absolute inset-0; observe the flex-1 parent wrapper
  useEffect(() => {
    if (!containerRef.current) return;
    const el = containerRef.current;
    const wrapper = el.parentElement ?? el;
    const { width: initW, height: initH } = wrapper.getBoundingClientRect();
    const chart = createChart(el, { ...CHART_OPTIONS, width: initW || 600, height: initH || 400 });
    chartRef.current = chart;
    const ro = new ResizeObserver(([entry]) => {
      const { width, height } = entry.contentRect;
      ...
      <div style={{ display: 'flex', flexDirection: 'column', flexShrink: 0 }}>
        ...
        <button
          ...
          style={{
            ...
            background: 'rgba(245,158,11,0.85)', border: '1px solid rgba(245,158,11,0.5)',
            borderRadius: 6, color: '#fff', fontSize: 12, fontWeight: 700, cursor: 'pointer',
            opacity: (loading || !underlying || !expiry || selected.length === 0) ? 0.4 : 1,
            transition: 'opacity 0.15s',
          }}
        >
          {loading
  ...
  return (
    <div className="flex flex-col h-full min-w-0 overflow-hidden">
      ...
      <div className="flex-1 flex gap-2 p-2 overflow-hidden" style={{ minHeight: 0 }}>
        {/* table */}
        <div className="shrink-0 rounded-xl overflow-hidden" style={{ width: 300, minWidth: 300, maxWidth: 300, minHeight: 0, height: '100%' }}>
          <StrikeTable
            strikes={strikes}
            selected={selected}
            strikeKeyMap={strikeKeyMap}
...
export default function StraddleChart({ instruments, visible = true }: Props) {
  ...
  return (
    <div className="flex flex-col h-full min-w-0 overflow-hidden" style={{ background: 'transparent' }}>
      ...
      <div className="glass-bar flex flex-nowrap items-center gap-3 px-4 shrink-0 overflow-x-auto" style={{ borderBottom: '1px solid rgba(255,255,255,0.07)', minHeight: 52, paddingTop: 8, paddingBottom: 8 }}>
        ...
        <div style={{ display: 'flex', flexShrink: 0 }}>
          <button
            ...
            style={{
              ...
              lineHeight: 1.5, border: '1px solid rgba(255,255,255,0.12)',
              borderRadius: '6px 0 0 6px', transition: 'all 0.15s',
              background: mode === 'straddle' ? 'rgba(255,255,255,0.10)' : 'rgba(255,255,255,0.03)',
              color: mode === 'straddle' ? '#D1D4DC' : '#787B86',
          ...
          <button
            ...
            style={{
              padding: '5px 14px', fontSize: 12, fontWeight: 600, cursor: 'pointer',
              lineHeight: 1.5, border: '1px solid rgba(255,255,255,0.12)',
              transition: 'all 0.15s', borderRight: 'none', whiteSpace: 'nowrap',
              background: mode === 'strangle' ? 'rgba(255,255,255,0.10)' : 'rgba(255,255,255,0.03)',
              color: mode === 'strangle' ? '#D1D4DC' : '#787B86',
            }}
          >
            Strangle
          </button>
          <button
            ...
            style={{
              padding: '5px 14px', fontSize: 12, fontWeight: 600, cursor: 'pointer',
              lineHeight: 1.5, border: '1px solid rgba(255,255,255,0.12)',
              borderRadius: '0 6px 6px 0', transition: 'all 0.15s', whiteSpace: 'nowrap',
              background: mode === 'calendar' ? 'rgba(255,255,255,0.10)' : 'rgba(255,255,255,0.03)',
              color: mode === 'calendar' ? '#D1D4DC' : '#787B86',
            }}
          >
            Calendar
          </button>
        ...
        {mode === 'straddle' && (
          <div style={{ display: 'flex', flexShrink: 0 }}>
            <button
              onClick={() => setStraddleMode('single')}
              style={{
                padding: '4px 10px', fontSize: 11, fontWeight: 600, cursor: 'pointer',
                lineHeight: 1.5, border: '1px solid rgba(255,255,255,0.08)',
                borderRadius: '5px 0 0 5px', transition: 'all 0.15s', borderRight: 'none',
                background: straddleMode === 'single' ? 'rgba(255,152,0,0.12)' : 'rgba(255,255,255,0.02)',
                color: straddleMode === 'single' ? '#FF9800' : '#52525b', whiteSpace: 'nowrap',
              }}
            >
              Single
            </button>
            <button
              onClick={() => setStraddleMode('multi')}
              style={{
                padding: '4px 10px', fontSize: 11, fontWeight: 600, cursor: 'pointer',
                lineHeight: 1.5, border: '1px solid rgba(255,255,255,0.08)',
                borderRadius: '0 5px 5px 0', transition: 'all 0.15s',
                background: straddleMode === 'multi' ? 'rgba(255,152,0,0.12)' : 'rgba(255,255,255,0.02)',
                color: straddleMode === 'multi' ? '#FF9800' : '#52525b', whiteSpace: 'nowrap',
              }}
            >
              Multi
            </button>
