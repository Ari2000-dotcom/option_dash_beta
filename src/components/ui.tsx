 ...
 * TradingView palette: #131722 / #1E222D / #2A2E39
 */
...
export function Select({ value, onValueChange, options, className, placeholder, disabled }: SelectProps) {
  return (
    <ShadSelect value={value} onValueChange={onValueChange} disabled={disabled}>
      <SelectTrigger
        className={cx(
          'h-7 px-2 rounded text-[12px] text-[#D1D4DC] bg-[#1E222D] border border-[#2A2E39] outline-none cursor-pointer transition-colors hover:border-[#363A45] disabled:opacity-40 min-w-[80px] focus:ring-0 focus:ring-offset-0',
          className,
        )}
      >
        <SelectValue placeholder={placeholder ?? '—'} />
      </SelectTrigger>
      <SelectContent className="z-[9999] bg-[#1E222D] border border-[#2A2E39] text-[12px] text-[#D1D4DC] shadow-[0_8px_32px_rgba(0,0,0,0.65)]">
        {options.map(o => (
          <SelectItem
            key={o.value}
...
export const NativeSelect = React.forwardRef<HTMLSelectElement, NativeSelectProps>(
  ({ options, placeholder, className, ...props }, ref) => (
    <select
      ref={ref}
      className={cx(
        'h-7 rounded px-2 text-[12px] text-[#D1D4DC] bg-[#1E222D] border border-[#2A2E39] outline-none cursor-pointer transition-colors hover:border-[#363A45] focus:border-[rgba(255,152,0,0.45)] appearance-none disabled:opacity-40',
        className,
      )}
      {...props}
    >
      {placeholder && (
        <option value="" disabled style={{ background: '#131722' }}>{placeholder}</option>
      )}
      {options.map(o => (
        <option key={o.value} value={o.value} style={{ background: '#131722' }}>
          {o.label}
        </option>
      ))}
    </select>
  ...
  ({ label, className, ...props }, ref) => {
    const input = (
      <input
        ...
        type="date"
        className={cx(
          'h-7 rounded px-2 text-[12px] text-[#D1D4DC] bg-[#1E222D] border border-[#2A2E39] outline-none cursor-pointer transition-colors hover:border-[#363A45] focus:border-[rgba(255,152,0,0.45)] disabled:opacity-40 [color-scheme:dark]',
          className,
        )}
        {...props}
      />
...
export const TextInput = React.forwardRef<HTMLInputElement, React.InputHTMLAttributes<HTMLInputElement>>(
  ({ className, ...props }, ref) => (
    <input
      ref={ref}
      className={cx(
        'h-7 rounded px-2 text-[12px] text-[#D1D4DC] bg-[#1E222D] border border-[#2A2E39] outline-none transition-colors hover:border-[#363A45] focus:border-[rgba(255,152,0,0.45)] placeholder:text-[#52525b] disabled:opacity-40',
        className,
      )}
      {...props}
    />
...
export const NumberInput = React.forwardRef<HTMLInputElement, NumberInputProps>(
  ({ onValueChange, className, ...props }, ref) => (
    <input
      ...
      className={cx(
        'h-7 rounded px-2 text-[12px] text-[#D1D4DC] bg-[#1E222D] border border-[#2A2E39] outline-none transition-colors hover:border-[#363A45] focus:border-[rgba(255,152,0,0.45)] disabled:opacity-40',
...
export const TextArea = React.forwardRef<HTMLTextAreaElement, React.TextareaHTMLAttributes<HTMLTextAreaElement>>(
  ({ className, ...props }, ref) => (
    <textarea
      ref={ref}
      className={cx(
        'rounded px-2 py-1.5 text-[12px] text-[#D1D4DC] bg-[#1E222D] border border-[#2A2E39] outline-none transition-colors hover:border-[#363A45] focus:border-[rgba(255,152,0,0.45)] placeholder:text-[#52525b] disabled:opacity-40 resize-y font-mono leading-relaxed',
        className,
      )}
      {...props}
    />
  ...
  ({ onClear, value, className, ...props }, ref) => (
    <div className="relative flex items-center">
      ...
      <input
        ...
        value={value}
        className={cx(
          'h-7 w-full rounded pl-7 pr-6 text-[12px] text-[#D1D4DC] bg-[#1E222D] border border-[#2A2E39] outline-none transition-colors hover:border-[#363A45] focus:border-[rgba(255,152,0,0.45)] placeholder:text-[#52525b] disabled:opacity-40',
          className,
        )}
        {...props}
      />
...
export function TabsList({ children, className }: { children: React.ReactNode; className?: string }) {
  return (
    <ShadTabsList
      className={cx(
        'flex gap-0.5 p-0.5 h-auto bg-[#1E222D] border border-[#2A2E39] rounded-md',
        className,
      )}
    >
      {children}
